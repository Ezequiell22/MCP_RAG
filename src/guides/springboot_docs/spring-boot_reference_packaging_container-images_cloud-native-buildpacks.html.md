...
If youâve ever used an application platform such as Cloud Foundry or Heroku then youâve probably used a buildpack.
...
For example, Cloud Foundryâs Java buildpack will notice that youâre pushing a `.jar` file and automatically add a relevant JRE.
...
|  |  |
| --- | --- |
|  | The [Paketo Spring Boot buildpack](https://github.com/paketo-buildpacks/spring-boot) supports the `layers.idx` file, so any [layer customization](efficient-images.html#packaging.container-images.efficient-images.layering) that is applied to it will be reflected in the image created by the buildpacks. |

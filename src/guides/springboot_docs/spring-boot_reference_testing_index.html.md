...
Test support is provided by two general-purpose modules â `spring-boot-test` contains core items and `spring-boot-test-autoconfigure` supports auto-configuration for tests â and several focused `-test` modules that provide testing support for a particular feature.
